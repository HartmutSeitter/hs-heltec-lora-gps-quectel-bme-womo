...
// ATTENTION the oder in bytes must be reversed defined (last byte form TTN is the first byte here)
static const u1_t PROGMEM DEVEUI[8]= { 0x1A, 0xAF, 0x05, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 }; // hs002
...
static const u1_t PROGMEM APPKEY[16] = { 0xEE, 0x3F, 0x10, 0x84, 0xA2, 0xC1, 0x6C, 0x78, 0x71, 0xE2, 0x1E, 0xBA, 0x03, 0xF0, 0xCE, 0xC9 };
...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
        ...
        case EV_TXCOMPLETE:
            ...
            if (LMIC.dataLen) {  
              ...
              for (int i = 0; i < LMIC.dataLen; i++) {
                ...
                rec_buffer[i] = LMIC.frame[LMIC.dataBeg + i];
              }
              Serial.print("\n");
              txStatus = TX_STATUS_UPLINK_ACKED_WITHDOWNLINK;
            }   
        ...
        */
        case EV_TXSTART:
            ...
            break;
            case 20:
            Serial.println(F("LoRa Join not working - either ttn key problem or no ttn gateway reachable"));
            txStatus = TX_STATUS_UNKNOWN;
            lora_txt_completion = 20;
            break;
        default:
    ...
    // set the global var ttn_txt_status to the event typ received - will be checked in main, 
    //   wether the txt was successful or not
    //txStatus1 = EV_TXSTART;
}
